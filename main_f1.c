...
struct boardinfo board_info = {
	.board_type	= BOARD_TYPE,   /*PXIO v1  10*/
	.board_rev	= 0,
	.fw_size	= APP_SIZE_MAX, /*PXIO v1 0xf000*/

	.systick_mhz	= OSC_FREQ,  /*PXIO v1 24M*/
};

static void board_init(void);   /*板上资源时钟开启，io口引脚配置*/

static void
board_init(void)
{
	/* initialise LEDs */
	rcc_peripheral_enable_clock(&BOARD_CLOCK_LEDS_REGISTER, BOARD_CLOCK_LEDS);   /*初始化LED时钟*/
	gpio_set_mode(BOARD_PORT_LEDS,
		      GPIO_MODE_OUTPUT_50_MHZ,
		      GPIO_CNF_OUTPUT_PUSHPULL,
		      BOARD_PIN_LED_BOOTLOADER | BOARD_PIN_LED_ACTIVITY);   /*配置LED引脚*/
	BOARD_LED_ON(
		BOARD_PORT_LEDS,
		BOARD_PIN_LED_BOOTLOADER | BOARD_PIN_LED_ACTIVITY);        /*点亮bootloader灯、状态指示灯*/

	/* if we have one, enable the force-bootloader pin */
#ifdef BOARD_FORCE_BL_PIN     /*强制引导引脚*/
	...
	rcc_peripheral_enable_clock(&RCC_APB1ENR, RCC_APB1ENR_PWREN | RCC_APB1ENR_BKPEN);   /*使能备份寄存器*/

#ifdef INTERFACE_USART     /*stm32f10x为串口2*/
...
/*board_deinit(void) 关闭资源时钟，重置io引脚配置*/
void
board_deinit(void)
{
	/* deinitialise LEDs */
...
should_wait(void)
{
	...
	PWR_CR |= PWR_CR_DBP;  /*PWR_CR = 1 << 8 允许写入RTC和备份寄存器*/

	if (BKP_DR1 == BL_WAIT_MAGIC) {   /*读取备份寄存器  BL_WAIT_MAGIC=0x19710317*/
		result = true;
		BKP_DR1 = 0;
	}
...
main(void)
{
	...
	timeout = BOOTLOADER_DELAY;  /*BOOTLOADER_DELAY=5000*/
#endif
...
# error I2C bootloader detection logic not implemented
	...
	if (should_wait()) {  /*读取备份寄存器，并且判断数据是否需要等待*/
	...
	clock_init();   /*配置bootloader模式下的时钟48Mhz*/

	/* start the interface */
	cinit(BOARD_INTERFACE_CONFIG, USART);  /*初始化串口，stm32F1只用串口1  115200 8 1 none*/
